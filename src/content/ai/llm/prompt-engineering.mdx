...
<Callout type="note">
Treat a prompt like you'd treat a function signature: inputs, expected output shape, and the constraints that make the output usable downstream. If you can't write down what a correct response looks like, you can't reliably prompt for it either.
</Callout>
...
<Callout type="tip">
Put the actual data (the ticket, the document, the transcript) after the instructions, not before. Instructions that come after a large block of data compete with everything the model just read for attention, and in practice get followed less reliably.
</Callout>
...
<ModelComparison
  models={[
    { name: "claude-opus-5", context: "1M", strengths: "Deepest reasoning, long multi-step tasks, ambiguous instructions", tradeoff: "Higher latency and cost for simple tasks" },
    { name: "claude-sonnet-5", context: "1M", strengths: "Balanced quality and speed, default for most production prompts", tradeoff: "Occasionally needs more explicit structure than Opus for edge cases" },
    { name: "claude-haiku-4-5-20251001", context: "200K", strengths: "Fast, cheap for classification and extraction", tradeoff: "Needs tightly structured prompts; weaker on multi-step reasoning" }
  ]}
/>
...
"Never do X" instructions are weaker than positive instructions describing what to do instead. A prompt that lists ten things not to do gives the model ten ways to accidentally violate one while satisfying the others. State the desired behavior directly wherever you can.
</Callout>
