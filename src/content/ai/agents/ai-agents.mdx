...
<Callout type="note">
If you can write down the exact sequence of steps in advance, you want a workflow, not an agent — a hardcoded pipeline with LLM calls at specific steps. Reach for an agent only when the task genuinely requires open-ended, model-driven exploration where the right sequence of actions isn't known ahead of time.
</Callout>
...
<Callout type="tip">
Not every tool result needs to stay in context forever. A large file read or search result that was only needed to inform one decision can often be cleared after that decision is made — keep the decision and its outcome, drop the raw data that produced it.
</Callout>
...
Don't rely on prompt instructions alone to keep an agent from calling a dangerous tool inappropriately ("only trigger_deploy if you're sure"). Enforce the boundary in code — gate the tool itself behind confirmation, rate limits, or scope restrictions — because a prompt instruction is a preference the model usually follows, not a guarantee.
</Callout>
