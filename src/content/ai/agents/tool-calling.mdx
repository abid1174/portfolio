...
<Callout type="note">
"The model called the tool" is a common but slightly misleading phrase. The model produced structured intent to call a tool. Nothing executes until your code reads that block and runs the corresponding function — which means tool calling is exactly as safe or unsafe as the code you write to handle it, not the model's judgment alone.
</Callout>
...
<Callout type="tip">
When an application has many tools, group related ones and consider only exposing a subset relevant to the current task. A model choosing between three well-differentiated tools is far more reliable than one choosing between thirty similarly named ones.
</Callout>
...
Idempotency matters more in tool-calling systems than it does in most user-facing APIs, because retries here are driven by a model's judgment, not a fixed retry policy you control. A network blip during a `charge_customer` call, followed by the model deciding to retry, should not double-charge — build idempotency keys into side-effecting tools from the start.
</Callout>
