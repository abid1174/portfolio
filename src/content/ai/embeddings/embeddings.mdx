...
<Callout type="warning">
  That snippet is deliberately illustrative, not a real embeddings call — chat completion models
  are not embedding models. Use a model built and trained specifically to output embeddings;
  mixing the two produces vectors with none of the geometric properties similarity search
  depends on.
</Callout>
...
<Callout type="insight" title="Normalization matters more than the metric">
  A common bug: mixing normalized query vectors with un-normalized document vectors, or vice
  versa. The similarity scores still come back as numbers that look plausible, they're just
  wrong — and nothing in the pipeline raises an error to tell you.
</Callout>
...
<Callout type="tip">
  Whatever embedding model you pick, embed queries and documents with the exact same model and
  the exact same preprocessing (lowercasing, whitespace handling, truncation length). Embeddings
  from two different model versions are not comparable, even if the model name looks the same.
</Callout>
