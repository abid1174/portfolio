...
<Callout type="note">
  Brute-force search is still the right choice for small collections. Below roughly
  10,000-50,000 vectors, scanning everything with a vectorized dot product is often faster in
  wall-clock terms than the overhead of building and querying an approximate index. Don't reach
  for a vector database before you've measured that brute force is actually too slow.
</Callout>
...
<Callout type="tip">
  Tune `ef_search` against a labeled eval set, not by feel. Push it up until recall stops
  improving meaningfully, then back off slightly for latency headroom — the recall curve is
  steep at first and flattens quickly, so the useful range is usually narrower than people
  expect.
</Callout>
...
<Callout type="insight">
  The metadata filtering story matters more than the raw ANN benchmark numbers for most
  applications. A database that's 10% slower on an unfiltered million-vector benchmark but
  supports fast native filtering will usually beat one that's faster unfiltered but forces you
  to over-fetch and filter in application code.
</Callout>
...
<Callout type="warning">
  Deletes in an HNSW graph are usually soft — the vector is marked removed but the graph
  structure and its links often aren't rebuilt immediately. Left unmanaged, a high-churn
  collection accumulates dead nodes that still cost memory and slow traversal, so plan for
  periodic compaction rather than assuming deletes are free.
</Callout>
