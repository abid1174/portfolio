...
<Callout type="note">
The single most common mistake is fine-tuning a model to "know" facts that change frequently — pricing, inventory, policy text — when those facts belong in a retrieval index that can be updated in seconds, not in a training dataset that requires a new run every time something changes.
</Callout>
...
<Tradeoff
  optionA="RAG"
  optionB="Fine-tuning"
  a={["Knowledge updates instantly by editing the index", "Costs extra tokens and a retrieval hop on every single call"]}
  b={["Lower per-call cost once trained — no extra context to send", "Every knowledge update requires a new training run and evaluation pass"]}
...
A smaller model that's cheap enough to run at high volume is often only viable after fine-tuning it on your specific task — a frontier model can succeed at that task zero-shot, but a small model may need the extra signal from fine-tuning to match it. That's a legitimate reason to fine-tune even for a "capability" problem rather than a "knowledge" one.
</Callout>

<ModelComparison
  models={[
    { name: "claude-opus-5", context: "1M", strengths: "Strong zero-shot performance on novel domains without fine-tuning", tradeoff: "Higher cost per call makes fine-tuning a smaller model more attractive at volume" },
    { name: "claude-sonnet-5", context: "1M", strengths: "Good balance for RAG pipelines — handles long retrieved context well", tradeoff: "Still benefits from fine-tuning for narrow, high-volume classification tasks" },
    { name: "claude-haiku-4-5-20251001", context: "200K", strengths: "Cheapest to run at scale, good fine-tuning target for narrow tasks", tradeoff: "Needs either strong retrieval context or fine-tuning to match larger models on complex tasks" }
  ]}
/>
