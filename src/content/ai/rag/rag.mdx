...
featured: true
related:
  ['ai/embeddings', 'ai/vector-databases', 'ai/rag-vs-fine-tuning', 'concepts/database-index']
---
...
<Callout type="note">
  RAG is not a database feature and not a single library — it's an architecture pattern built
  from three pieces you already understand: search, prompt construction, and generation. Most of
  the engineering effort goes into the search step, not the LLM call.
</Callout>
...
<Callout type="tip" title="Chunk on structure first">
  Splitting on a fixed token count alone tends to cut sentences and tables in half. Split on
  document structure — headings, paragraphs, list boundaries — and only fall back to a token cap
  when a structural unit is too large on its own.
</Callout>
...
<Callout type="warning">
  Adding more chunks to the prompt is not a free fix for weak retrieval. Every irrelevant chunk
  you stuff into context costs tokens, adds latency, and gives the model more chances to anchor
  on the wrong passage. Tune the retriever before you widen `k`.
</Callout>
