...
<Callout type="warning">
  Retrying a failing call feels like resilience, but blind retries during an outage are the
  single most common way to turn a degraded dependency into a fully dead one — you're adding
  load precisely when it has the least capacity to handle it.
</Callout>
...
<Callout type="tip">
  Start from the dependency's own error budget and typical latency, not a round number picked
  out of habit. A payment provider with a documented 99.9% SLA should trip on a failure rate
  meaningfully above 0.1%, sampled over a window long enough to not be dominated by single-digit
  request counts.
</Callout>
...
<Callout type="insight">
  A tripped-then-recovered breaker means some calls definitely failed and some may have
  partially succeeded before the trip. Anything a circuit breaker wraps that isn't safely
  retriable — a payment call without an idempotency key, for instance — can leave you with
  duplicated side effects once the breaker closes again and retries resume. Circuit breakers
  assume the calls behind them are safe to retry; make sure that's actually true before relying
  on one.
</Callout>
