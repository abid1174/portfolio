...
<Callout type="note">
HTTP/2 also introduced server push — letting the server proactively send resources it knows the client will need. It looked great on paper and turned out badly in practice: servers routinely pushed things the browser already had cached, wasting bandwidth. Chrome and most other browsers removed support for it; the technique that actually stuck is preload hints (`Link: rel=preload`), which tell the client what to fetch without forcing it.
</Callout>
...
This is the core reason HTTP/2 didn't fully deliver on mobile: a 1% packet loss rate on a connection carrying 100 multiplexed streams can stall all 100 of them waiting on one retransmission, even though 99 streams had nothing to do with the lost packet.
</Callout>
...
Enabling HTTP/2 or HTTP/3 on your server is close to free — most CDNs and load balancers do it automatically. But if your application still ships giant server-rendered HTML with a handful of requests per page, the protocol upgrade won't move your numbers much. The wins are largest for request-heavy, latency-sensitive, or mobile-heavy workloads.
</Callout>
