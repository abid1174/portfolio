...
<Callout type="warning" title="Skip implicit and password grants">
  Both hand the client a token directly in a redirect fragment or a form post, with no exchange
  step and no client authentication. OAuth 2.1 removes them outright. If you're starting
  something new, authorization code with PKCE covers essentially every case that implicit used
  to.
</Callout>
...
<Callout type="insight">
  A good mental model: scopes are what the user agreed to when they clicked "Allow."
  Authorization logic on the resource server is what actually gets enforced. Never assume a
  token's scope alone proves the request should succeed.
</Callout>
...
<Callout type="note">
  Access token: proves the bearer can call an API. ID token: proves who authenticated. Mixing
  these up — using an access token to identify a user, or an ID token to call an API — is a
