...
<Tradeoff
  ...
  b={["Stateless validation, no network hop", "Cannot be revoked before expiry without extra state", "Payload is visible to anyone holding the token"]}
...
The JWT spec technically allows an unsigned token via `alg: none`. Older libraries trusted whatever algorithm the token itself claimed, which let an attacker hand-craft a token, set `alg` to `none`, and skip signature verification entirely. Always pin the expected algorithm on the verifying side instead of trusting the header.
</Callout>
...
<Callout type="danger">
Never put secrets, passwords, or anything sensitive directly into a JWT payload. If you genuinely need confidentiality, use a JWE (JSON Web Encryption) instead, or just keep the sensitive data out of the token and look it up server-side using the `sub` claim.
</Callout>
...
The refresh token is where your actual session state lives. The access token is closer to a temporary, cryptographically-backed cache of "this session was valid as of a few minutes ago."
</Callout>
