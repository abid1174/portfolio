...
featured: false
related:
  ['concepts/database-index', 'concepts/consistent-hashing', 'system-design/rate-limiter']
---
...
<Callout type="warning">
  Stampedes often masquerade as a database performance problem. The graphs show a spike in DB
  load and slow queries, and the instinct is to add an index or scale the database — when the
  actual fix belongs entirely in the caching layer.
</Callout>
...
<Callout type="insight">
  Notice the last row: the simplest fix for a lot of stampede-prone caches is to stop expiring
  on a timer at all. If you can invalidate the cache precisely when the underlying data changes
  — on the write path — you remove the recurring expiration event that causes stampedes in the
  first place. TTLs are a safety net for invalidation you don't trust, not a substitute for it.
</Callout>
