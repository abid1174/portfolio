...
featured: true
related:
  ['concepts/cache-stampede', 'system-design/url-shortener', 'concepts/consistent-hashing']
---
...
<Callout type="note">
  Not every index is a B-tree. Hash indexes are faster for pure equality lookups but useless for
  ranges. GIN and GiST indexes handle full-text search, arrays, and geometric data. Postgres
  also supports BRIN indexes for huge, naturally-ordered tables like time-series data, where you
  don't need per-row precision, just per-block. Default to B-tree unless you have a specific
  reason not to.
</Callout>
...
<Callout type="tip">
  Covering indexes are a strong lever for read-heavy, narrow queries — dashboards, list views —
  but resist the urge to make every index cover every possible query. Each additional included
  column grows the index and adds write overhead for a benefit that only pays off if that exact
  query shape actually runs often.
</Callout>
...
<Callout type="warning">
  A common failure mode is a table with a dozen indexes added incrementally by different
  features, each individually justified, collectively turning every write into a slow,
  multi-index update. Periodically check for genuinely unused indexes (`pg_stat_user_indexes` in
  Postgres) and drop them — an index nobody queries against is pure write overhead.
</Callout>
