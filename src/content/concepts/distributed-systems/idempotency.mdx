...
<Callout type="note">
"Idempotent by spec" describes intended semantics, not a guarantee your implementation honors them. A `PUT` handler that appends to a log instead of replacing a value is not actually idempotent no matter what the HTTP method suggests. The method is a contract with the caller, not something the framework enforces for you.
</Callout>
...
async function handleCharge(req: Request) {
  ...
  await db.idempotencyKeys.update(
    { key },
    { status: "completed", response: charge }
  );
...
<Callout type="tip">
Generate idempotency keys client-side with a UUID, and hold onto the same key across retries of the same user action — regenerating it on retry defeats the entire mechanism, since the server sees what looks like a brand-new operation each time.
</Callout>
...
A distributed cache with `SETNX` works the same way as a unique constraint — the point is that whatever store you use, the "claim this key" step has to be a single atomic operation, not a read followed by a separate write.
</Callout>
