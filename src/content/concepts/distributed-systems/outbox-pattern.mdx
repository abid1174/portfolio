...
featured: false
related:
  ['concepts/idempotency', 'concepts/consistent-hashing', 'system-design/notification-system']
---
...
<Callout type="warning">
  Every consumer of outbox-published events has to be idempotent — safe to process the same
  event twice without double-charging a card, double-sending an email, or double-decrementing
  inventory. Idempotency isn't optional here; it's the other half of the pattern.
</Callout>
...
<Callout type="insight">
  Two-phase commit looks like the "correct" textbook answer, but almost nothing you actually
  want to use in production — Kafka, SQS, RabbitMQ — participates in a distributed transaction
  coordinator. The outbox pattern sidesteps the whole problem by only requiring atomicity within
  the database you already have transactions in.
</Callout>
...
<Callout type="tip">
  If you need strict per-aggregate ordering, partition the broker topic by `aggregate_id` and
  have the relay preserve `created_at` order within each aggregate — most brokers only guarantee
  ordering within a partition, not across an entire topic.
</Callout>
