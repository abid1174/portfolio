...
featured: true
related:
  ['concepts/cache-stampede', 'concepts/database-index', 'system-design/file-storage-system']
---
...
<Callout type="warning">
  This is the exact mechanism behind painful "we added a cache node and latency got worse before
  it got better" incidents. Modulo hashing makes cluster resizing an event that punishes you for
  scaling.
</Callout>
...
<Callout type="insight">
  The O(log M) lookup cost is a real trade, not a rounding error, but it's almost always worth
  paying: a small constant-factor cost on every lookup versus a cluster-wide cache stampede on
  every resize is not a close call at production scale.
</Callout>
...
<Callout type="tip">
  Redis Cluster's approach — hashing keys into 16,384 fixed hash slots and assigning slots to
  nodes — sidesteps needing a live ring lookup at request time, at the cost of a coordination
  step (resharding) to move slots between nodes explicitly rather than having membership changes
  reroute keys automatically. It's a related idea optimized for a different operational model.
</Callout>
