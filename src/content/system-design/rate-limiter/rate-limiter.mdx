...
related: ["concepts/circuit-breaker", "concepts/consistent-hashing", "system-design/url-shortener"]
---
...
<Callout type="note">
We're designing the general-purpose gateway rate limiter here, not a specialized abuse/fraud detection system — this component enforces stated, published limits fairly and predictably; it isn't trying to detect malicious intent.
</Callout>
...
<Callout type="tip">
Decide up front whether being slightly over the limit under contention is acceptable. Most API rate limiters intentionally allow soft overshoot (a distributed counter with a few milliseconds of replication lag might let a client through at 1,010 requests instead of exactly 1,000) in exchange for not needing expensive cross-node coordination on every request. This single decision shapes the whole algorithm choice below.
</Callout>
...
<Capacity
  ...
  items={[
    ...
    { label: "Counter memory", value: "~3.2 MB", note: "2 tiers x 100K clients x 2 counters x 8B" },
    { label: "Central store ops (naive)", value: "500,000/s", note: "1 network hop per request if not localized" },
    { label: "Local check cost", value: "sub-microsecond", note: "vs ~1ms+ network round trip" }
  ]}
/>
...
<Decision title="Sliding window counter for sustained limits, token bucket for burst limits" choice="Two algorithms, one per tier, rather than a single unified approach">
...
If the product requirement is a hard, exact limit — for example, enforcing a paid quota tied to billing — this local-cache approach is the wrong choice; that case needs every check to go through a single authoritative counter (accepting the latency and throughput ceiling that comes with it), or a token-based pre-allocation scheme where instances claim blocks of quota from a central store up front rather than checking per-request.
</Callout>
...
<Callout type="danger" title="Never let the rate limiter become a single point of failure for the whole gateway">
The most important property of this design is that every failure mode degrades toward "slightly less accurate limiting," never toward "the gateway stops serving requests." A rate limiter that fails closed on its own dependency outage has turned a component meant to protect availability into the biggest threat to it.
</Callout>
...
<Tradeoff
  ...
  a={["Exact, immediately consistent limits", "Hard latency floor of a network round trip on every request", "Redis becomes a scaling ceiling and a single dependency on the hot path"]}
  b={["Sub-microsecond hot-path check, scales with gateway fleet size", "Soft overshoot possible — up to roughly instanceCount x limit in a worst case", "More moving parts: sync workers, eviction, staleness to reason about"]}
