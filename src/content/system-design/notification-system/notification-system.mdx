...
<Callout type="note">
We're scoping this to transactional and triggered notifications, not a full marketing campaign/segmentation engine. Bulk marketing sends share the dispatch pipeline but have relaxed latency requirements and their own opt-out rules.
</Callout>
...
Not every notification deserves the same latency budget. A 2FA code needs to land in under 10 seconds or the user gives up; a weekly digest email can sit in a queue for an hour. Splitting classes early avoids over-provisioning the fast path to serve slow traffic.
</Callout>
...
<Decision title="Deduplicate at the dispatcher" choice="Redis SETNX on delivery_id with a 24h TTL, checked before every provider call">
...
Don't let "priority" become a single integer field on a shared queue with client-side sorting. Most managed queues don't guarantee priority ordering under load, and a busy queue means your "high priority" message still waits behind everything ahead of it. Physically separate queues with separate consumers are the only reliable way to protect the critical path.
</Callout>
...
We fail *open* for critical security notifications during a preference-service outage — a login alert should reach the user even if we can't confirm their SMS opt-in status. We fail *closed* for marketing. This has to be decided per category ahead of time; guessing during an incident means someone picks wrong under pressure.
</Callout>
...
<Tradeoff
  ...
  a={["Replayable log — reprocess a bad deploy's worth of events", "Natural fit if other services already consume notification events", "You operate partitioning, rebalancing, and consumer lag"]}
  b={["Fully managed, near-zero ops", "Built-in dead-letter queues and visibility timeouts", "No replay after a message is acked — a bug that mis-processes a batch is unrecoverable"]}
