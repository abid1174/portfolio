...
related:
  ['concepts/consistent-hashing', 'concepts/database-index', 'system-design/url-shortener']
---
...
<Callout type="note">
  We're scoping out real-time collaborative editing (multiple people editing the same document
  simultaneously, character by character) — that's a genuinely different problem (operational
  transforms / CRDTs) layered on top of a file storage system, not a core requirement of file
  storage and sync itself.
</Callout>
...
- Total storage: 10,000,000 × 50 GB ≈ 500 PB in principle, but real-world dedup and the fact that most users are well under the average brings actual stored bytes down substantially — we'll use the stated 500 TB _unique-content_ scale target as the practical planning number after estimating meaningful dedup savings from common files (OS files, popular documents, stock photos) offsetting outliers with genuinely large unique libraries.
...
<Capacity
  ...
  items={[
    ...
    {
      label: 'Upload QPS (peak)',
      value: '~56/s files, ~200-300/s chunks',
      note: '200K uploads/hour',
    },
...
<Callout type="tip">
  Content-defined chunking (splitting on byte patterns rather than fixed offsets, similar to
  what rsync uses) improves dedup rates for files that get small edits in the middle — a
  fixed-offset scheme means one inserted byte shifts every subsequent chunk boundary and
  destroys dedup for the rest of the file. It's more CPU-intensive client-side, so it's a
  reasonable upgrade path once basic fixed-size chunking is proven out, not a day-one
  requirement.
</Callout>
...
<Decision
  title="Content-addressed chunk storage with server-side existence check"
  choice="Client hashes locally, server confirms which hashes already exist before any bytes are sent"
>
  An alternative is server-side dedup after full upload (accept the bytes, hash them, discard if
  a duplicate exists) — simpler to implement, but wastes exactly the upload bandwidth we're
  trying to save, which defeats much of the point for large files. Checking before transfer
  costs one extra round trip on the upload-initiation call but is trivial next to the bandwidth
  saved on a multi-gigabyte duplicate.
</Decision>
...
<Callout type="danger" title="Never let a storage-cost optimization risk data loss">
  Deduplication and garbage collection touch shared, referenced data by definition — the failure
  mode to design against above all others is deleting a chunk that's still referenced by some
  user's file because a ref count was wrong. The mitigation is boringly conservative on purpose:
  garbage collection always re-verifies live references immediately before a physical delete,
  and never deletes based on a cached or eventually-consistent count alone.
</Callout>
