...
related: ["concepts/consistent-hashing", "concepts/idempotency", "system-design/notification-system"]
---
...
<Callout type="note">
We're scoping out end-to-end encryption key management and media (image/video) handling in detail — both are real and substantial subsystems, but the message-routing and delivery-guarantee core described here is unaffected by whether the payload is plaintext or a ciphertext blob, or whether it's a text string or a media reference.
</Callout>
...
We only need ordering guarantees within a single conversation, not a global total order across all conversations in the system. This is the difference between a tractable per-conversation sequence counter and a genuinely hard distributed consensus problem — don't over-build this.
</Callout>
...
<Capacity
  ...
  items={[
    ...
    { label: "Message storage / year", value: "~182 TB", note: "250 bytes/message, pre-replication" }
  ]}
/>
...
<Decision title="Shard the sequencer by conversation_id, not globally" choice="Consistent hashing of conversation_id to sequencer shard, each shard owning an independent counter space">
...
The pattern across receipts and typing indicators is the same: identify which parts of the "message" concept actually need the strong durability and ordering guarantees (message content) and which only need best-effort, ephemeral delivery (typing, and largely receipts) — then give each the guarantee it needs, not the strongest guarantee available. Treating everything with message-content-level rigor is the most common way these systems become slower than they need to be.
</Callout>
...
Don't let "the recipient is offline" branch into different code from "the recipient is online but the gateway push failed." Both are just "the message didn't reach a live connection right now," and the recovery is identical: it's durably stored, and the client will catch up via sequence range read on its next successful connection. Two code paths for the same outcome is where subtle message-loss bugs get introduced.
</Callout>
...
<Tradeoff
  ...
  a={["True bidirectional push, lowest latency for server-initiated messages", "Higher per-connection resource cost, needs sticky routing infrastructure", "Requires a connection-aware fan-out/routing layer"]}
  b={["Works everywhere without special infrastructure, simpler load balancing", "Higher latency (poll interval) and higher request overhead per message", "Doesn't scale well past moderate concurrency due to repeated connection setup"]}
