...
related: ["concepts/consistent-hashing", "concepts/database-index", "system-design/rate-limiter"]
---
...
<Callout type="note">
We're deliberately not designing for password-protected links or link-level access control here — that's a real feature in production shorteners, but it doesn't change the core system shape, so it's out of scope for this write-up.
</Callout>
...
<Capacity
  ...
  items={[
    ...
    { label: "Redirect QPS (peak)", value: "~8,000/s", note: "100:1 read/write, 2x daily peak" },
...
<Decision title="Counter-based generation over hashing" choice="Snowflake-style distributed id generator, base62-encoded">
...
<Callout type="tip">
Set a short TTL (minutes, not hours) even though `long_url` almost never changes after creation. The TTL isn't there for correctness — it's there so a link that suddenly goes viral doesn't permanently occupy cache memory it no longer needs once traffic fades, and so an expired or deleted link's stale mapping ages out on its own instead of needing an active eviction on delete.
</Callout>
...
<Callout type="warning" title="Never let analytics durability requirements leak into the redirect path">
The single most common design mistake in a URL shortener is writing the click event synchronously before issuing the redirect. It couples the most latency-critical path in the system to the durability guarantees of an analytics pipeline that doesn't need them. Keep them decoupled from day one — retrofitting this split under load is painful.
</Callout>
...
<Tradeoff
  ...
  a={["No extra service to run", "Collision-check-and-retry loop needed on every write", "Collision rate worsens as the table fills"]}
  b={["Zero collisions by construction, predictable write latency", "One more small stateful service to operate", "Straightforward to reason about and audit"]}
